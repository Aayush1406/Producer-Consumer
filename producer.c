...
(created by Parent using fork() fun)Child Process - Consumer
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
//argc = argument cnt = 4 (0 - Program name, 1 - buffSize, 2 - itemCnt, 3 - randSeed [./myProgram 100 500 42])
int main(int argc, char* argv[])
{
    pid_t pid;
    int bufSize; // Bounded buffer size. (no. of itm that can fit in. This cannot exceed 1000)
    int itemCnt; // Number of items to be produced. (mostly ItemCnt >> bufSize)
...
//atoi - ASCII to int (Converts "100"-> 100)
bufSize = atoi(argv[1]);
itemCnt = atoi(argv[2]);
