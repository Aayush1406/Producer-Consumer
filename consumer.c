...
Contains the same code as the Parent but in the different address space
*/
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
...
int main()
{
    const char *name = "OS_Aayush"; // Name of shared memory block to be passed to shm_open
    ...
    int ans = ftruncate(shm_fd,SHM_SIZE);
    if(ans==-1){
